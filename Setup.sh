...
bluw_tool_infra() { load_config; ensure_privileges; detect_os; run_step "install_base" install_base; run_step "install_docker" install_docker; if ! _compose_cmd >/dev/null; then echo "Compose indisponível"; exit 1; fi; if [ -z "${LETSENCRYPT_EMAIL-}" ] || [ -z "${PORTAINER_DOMAIN-}" ]; then echo "Variáveis ausentes"; exit 1; fi; validate_config; run_step "open_ports" open_ports; run_step "ensure_network" ensure_network; run_step "write_files" write_files; run_step "compose_up" compose_up; run_step "wait_traefik" wait_container traefik; run_step "wait_portainer" wait_container portainer; run_step "wait_tls" wait_tls; run_step "report" generate_report; echo "HTTPS em https://$PORTAINER_DOMAIN"; }
